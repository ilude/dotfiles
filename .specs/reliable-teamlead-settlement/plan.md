...
- Assumptions: One explicit modern Team Lead continuation is sufficient; declared read targets validate authority but never grant or widen it; filesystem containment reuses the existing canonical real-target and nearest-existing-ancestor rules; and the process-local run manager remains authoritative for atomic continuation issuance and consumption.
...
- [ ] **T1: Establish the outcome and budget contracts with one executable slice**
  ...
  - Done when: Exhaustive reducer cases, short/normal deadline calculations, continuation issuance, atomic first consumption, concurrent or second-use rejection, expiry, cancellation, and authority-narrowing rejection pass, and the shared contract typechecks before integration expands.
