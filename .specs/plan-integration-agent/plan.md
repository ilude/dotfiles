...
- Worktree and integration target: originating checkout `C:/Users/mglenn/.dotfiles`, branch `main`. On execution, create a dedicated task worktree/branch and record its actual path and branch here before editing.
...
- [ ] **T1: Define and test the bounded closeout state machine**
  - Depends on: none.
...
- [ ] **T2: Add the Integrator role and role-scoped closeout procedure**
...
- [ ] **T3: Permit validated Integrator closeout without Damage Control prompts**
...
- [ ] **T4: Wire plan execution and documentation to Integrator-owned closeout**
...
- Status: ready for execution when separately authorized.
- Completed work and evidence: planning only. Confirmed the existing role-definition mechanism, `/do-it` closeout contract, Damage Control modes/bypass boundaries, script-trust behavior, and Git preservation guidance. User settled automatic conditional stash authority, conflict boundaries, helper-based Damage Control handling, Luna-high Integrator defaults, `--no-merge` behavior, task-branch preservation, and parent-owned final reporting.
- Next: on execution authorization, create the dedicated task worktree, record its target, consult Strategist, and begin T1 before dependent role and Damage Control wiring.
- Blockers/open decisions: none.
- Verification limits: no Integrator, helper, authority handoff, or quiet Damage Control path has been implemented or tested. The motivating manual closeout is evidence of the workflow need, not validation of this design.
