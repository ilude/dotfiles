...
- [ ] **T3: Finalize `/plan-it` with assessment, clipboard delivery, and a final command row**
  - Depends on: T1
...
- [ ] Focused tests prove the shared TUI-only entries remain outside model context, one representative command acknowledges before slow work without editor mutation, and ready `/plan-it` finalization copies and appends the exact command once while one stale or failed path does not.
- [ ] One interactive `/plan-it` smoke shows the four requested assessment dimensions followed by the colored plain command entry; scoped inspection of every `registerCommand()` finds no inconsistent non-interactive surface and retains UI needed for interaction or cancellation.
- [ ] `rg -n 'registerCommand\("prd-it"' pi/extensions` returns no matches, current command-facing wording no longer presents `/prd-it` as available, focused Vitest files and `pnpm run typecheck` pass, and `git diff --check` reports no whitespace errors or unrelated changes.
