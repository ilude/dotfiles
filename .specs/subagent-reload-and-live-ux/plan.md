...
- Remaining coordination risk: the revised T3 blocker is resolved. The model-backed T5 child connected but timed out on both Herdr 0.8.2 and verified 0.9.0 in `phase: starting` before its first turn; attached-client T5 has not run. Maya's developer process is under direct user intervention and is not coordinator-controlled; no task-owned test resources remain.
