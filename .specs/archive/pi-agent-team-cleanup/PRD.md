...
Superseded on 2026-05-11 by `.specs/pi-control-plane-consolidation/plan.md`, which rolls the remaining agent team cleanup work together with the related `/branch` and task control-plane work.

## Current Implementation Check

Checked on 2026-05-11:

- `pi/agents/` exists and many agents already have `roleType` metadata.
- `pi/multi-team/agents/` still exists with active duplicate agent definitions, so canonical-source cleanup is not complete.
- `pi/extensions/agent-team.ts` still implements/advertises `/team`, so `/team` removal and single-surface `subagent` dispatch are not complete.
- Lead metadata such as `team` and `.pi/multi-team/*` references still appears in `pi/agents/*.md`, so misleading/advisory metadata cleanup is not complete.
- Existing code is therefore partial prerequisite state only; this PRD still needs a plan and implementation before it can be considered completed.
- Validation evidence: `make check` passed on 2026-05-11, confirming current code health but not this PRD's acceptance criteria.
