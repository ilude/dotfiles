...
- [x] **T1: Define a race-safe parent-question lifecycle in the subagent runtime**
  - Depends on: none.
  ...
  - Evidence: Implemented in `rpc.ts` with separate question resolution history, first-resolution-wins answer/cancellation semantics, accurate late-resolution errors, pending-question lifetime, and cancellation distinct from assignment cancellation. Focused messaging/runtime tests pass.

- [x] **T2: Deliver questions through an origin-scoped local parent mailbox**
  ...
  - Evidence: Implemented in `runtime.ts` with origin-scoped question mailbox delivery independent of attached waits, queued/delivered resolution handling, duplicate suppression, and concise post-delivery resolution updates. Focused runtime tests pass.

- [x] **T3: Make the visible child question self-explanatory and conversationally resolvable**
  ...
  - Evidence: Implemented in `subagent-child.ts`, `visible.ts`, and presentation helpers. Child results include complete bounded question text and request ID; ordinary visible input preserves pending requests; the child model can explicitly cancel a question. Focused child and presentation tests pass.

- [x] **T4: Integrate the behavior contract, regression suite, and operator documentation**
