...
- In scope: `pi/lib/plan-state.ts`, `pi/lib/workflow-commands/plan-lifecycle.ts`, `pi/skills/workflow/plan-it.md`, `pi/skills/workflow/do-it.md`, `pi/extensions/workflow-commands.ts` (live-task gate and reviewer prompt text only), `pi/extensions/active-turn-compaction.ts`, `pi/skills/herdr/SKILL.md` and a new `pi/skills/herdr/references/automation.md`, focused Vitest files for the parser, validator, and compaction template, and the owning contracts `workflow-lifecycle.md`, `session-lifecycle.md`, and `subagents-and-tasks.md` where behavior changes.
- Out of scope: Changing `/plan-it` modes, review counts, or the standard default; Herdr product code under `pi/extensions/subagent/`; damage-control rules; `pi/AGENTS.md` (failure classification already exists at lines 26-28); closeout-tool error semantics and state gating (`plan_archive` and `workflow_complete` already return `isError` on failed preconditions and are already activated per `/do-it` mode); a `subagent_read` external-evidence option (convention only: copy evidence into `.tmp/evidence/` before delegating); A shared live-Herdr test helper and migration of the Herdr live tests to one PI_LIVE gate; those files exist only on workflow/herdr-visible-subagents and are recorded as a follow-up in that plan's T11. Automatic cancellation of children on user correction (prose in `do-it.md` only); goal or `/goal --unattended` semantics; friction telemetry gaps; editing any archived plan.
- Preserve: Existing plans without live tagging parse and execute unchanged; `Verify:` presence checks, the 1-16 task bound, the review-role lifecycle and its four-record limit, `/do-it` continuation and closeout, the compaction token budget and existing bullets, `HERDR_ENV=1` gate, and every currently passing test.
...
- [x] **T3: Give reviewers a verification-design rubric and teach /do-it the stop rules**
...
- Smaller-model work: T1 `automation.md` authoring from the facts listed in the task, and the `do-it.md` edits in T3 are bounded leaf packages; T2 parser and validator changes and the T3 reviewer rubric wording remain root-owned because they gate acceptance for every future plan.

## Validation

- [x] `cd pi && pnpm test plan-state.test.ts plan-lifecycle.test.ts workflow-commands.test.ts active-turn-compaction.test.ts` passes.
- [x] `cd pi && pnpm run typecheck` passes.
- [x] A scratch plan with `Verify: live` and no `Max attempts` is rejected by readiness validation; the same plan with all three fields and a ledger is accepted; an untagged existing plan is accepted; a scratch plan touching `modules/onclave/` without a `Repositories:` bullet is rejected.
- [x] The adversary review of the two-claim model-dependent scratch task returns a supported finding naming the task and rubric items 3 and 4.
- [x] `git diff --check` passes and no file contains non-ASCII punctuation.
...
- Blocker: None.
- Next: None.
- Result: T1 focused compaction tests and Pi typecheck passed after restoring the documented worktree-local dependency links.
- Result: T2 parser, lifecycle, and workflow-command tests passed with Pi typecheck; coverage includes live metadata, ledger caps and authorization, rejected outcomes, and multi-repository declarations.
- Result: T3 prompt guidance, reviewer directive, scratch-plan validation, and ASCII checks passed; the adversary returned the required supported finding.
- Review excerpt: T1 failed rubric items 3 and 4 because `Done when` and `Verify` depend on what the child voluntarily chooses, and the allowed-edit and forbidden-action outcomes can fail independently; rewrite as separate model-independent tasks with their own live bounds.
