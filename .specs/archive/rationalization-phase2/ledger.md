...
| Ingestion-time reduction of routine tool results | Fresh evidence loses fidelity and payload prefixes churn | `research/context-reduction-research.md` and Pi `ContextEvent` API | Routine Bash output over 240 bytes was reduced before its first model-visible turn | Replace with threshold batches | Routine results stay whole until Pi reports 50% usage; batches exclude the five newest results, reclaim at least about 5,000 tokens, and wait for 5,000 more Pi-accounted tokens before another batch. Extreme 64 KiB Bash output still clamps on ingestion. Session-file markers preserve recovery, transient worker failures retry, and session-tree changes restore recent results to full fidelity. | T13 | Executed |
