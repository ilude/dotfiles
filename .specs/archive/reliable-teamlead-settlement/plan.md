...
- [x] **T1: Establish the outcome and budget contracts with one executable slice**
...
- [x] **T2: Integrate continuation, cutoff, and read-target enforcement**
  - Depends on: T1
...
- [x] **T3: Align operator surfaces and the stable contract**
  - Depends on: T2
...
- [x] Run `cd pi && pnpm test subagent.test.ts subagent-tree-runtime.test.ts subagent-run-manager.test.ts workspace-policy.test.ts subagent-control.test.ts subagent-workflow.test.ts orchestration-stats.test.ts orchestration-telemetry.test.ts && pnpm run typecheck`; 196 focused tests and the extension typecheck pass, covering mixed outcomes, consume-once continuation, cutoff races, canonical target rejection, cancellation, and telemetry separation.
...
- State: Complete.
- Blocker: None.
- Evidence: 196 focused tests pass across the eight specified files; `pnpm run typecheck` passes; `git diff --check` passes.
