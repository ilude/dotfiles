...
- [ ] **T1 — Establish worktree and prove the layout seam**
...
- [ ] **T2 — Add stable human identity and display metadata**
...
- [ ] **T3 — Render launch, activity, control, and outcome details**
  - Depends on: T2.
...
- [ ] **T4 — Integrate stable shared pane placement and titles**
...
- [ ] **T5 — Validate integrated UX and document actual behavior**
  - Depends on: T3, T4.
...
New presentation/layout tests are selected by the `subagent` filter. Add one proposed opt-in `tests/subagent-ux-live.test.ts`, using `PI_SUBAGENT_UX_LIVE=1 pnpm test subagent-ux-live.test.ts`, to own the isolated inert geometry and bounded real-child cases above. This file/flag do not exist yet. Reuse existing named-server/plugin isolation instead of building a general harness. Geometry checks populate 1/4/5/8/9/17 positions with inert processes, not seventeen models. The single real child checks actual bootstrap composition. Never stop the shared server, mutate production plugin links, or move the user's live panes for testing.
...
- Status: plan ready; implementation not authorized by the planning request.
- Completed: source/CLI inspection, current versus obsolete lifecycle reconciliation, and owning-document correction.
- Next: T1 after execution authorization.
- Technical verification remaining: current isolated Herdr swap/row/overflow composition; bounded by T1.
- Operator decisions: no repeated pane-closure question. Overflow interpretation and sizing defaults are explicit above rather than hidden implementation choices.
- Actual checks: planning only; no runtime or live model validation performed.
