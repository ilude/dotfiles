...
- JavaScript/TypeScript package-manager policy: prefer `bun` for general JS/TS work; never use `npm` or create/commit `package-lock.json`.
- Pi TypeScript work is pnpm-only, no exceptions. `pi/package.json` is the single source of truth for Pi TypeScript dependencies, typecheck, and Vitest. Do **not** use `bun add`, `bun install`, `bun run`, or `bun test` for Pi-related TypeScript packages/tests. Run `cd pi && pnpm install --frozen-lockfile && pnpm run typecheck` and `cd pi && pnpm test`. For a single Vitest file, pass the filter directly to the pnpm script, e.g. `cd pi && pnpm test operator-status.test.ts`; do **not** insert `--` before the file filter, because this repo's script passes it through to Vitest and runs the full suite. `@earendil-works/*` and `@sinclair/*` are intentionally absent from `pi/package.json` -- they are symlinked from pnpm-global into `pi/node_modules` by `scripts/pi-deps-link-setup` so they always match the installed pi binary version.
- Tests use `pytest`.
