...
- **NO EM-DASHES OR EN-DASHES IN FILES** - When writing code, comments, docs, or commit messages, use ASCII punctuation only: `--` (double hyphen) or `-` for ranges/breaks. Do NOT use the em-dash or en-dash characters. Reason: round-trips through Windows-1252 / cp1252 (common in Windows toolchains and some terminals) corrupt them to mojibake (`a-tilde-euro-quote` sequences), which then breaks subsequent Edit string-matching and pollutes diffs. Applies to all file content; chat replies to the user may use them freely.
