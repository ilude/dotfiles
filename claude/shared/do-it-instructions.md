...
5. **Validation Contract gate** — check whether the plan contains a `## Validation Contract` section. If present, treat it as authoritative for completion and archiving requirements. Extract required automated validation commands, task-specific verification, whether manual validation is required, whether deployment validation is required, and the archive rule. If absent, continue using the legacy gates below; do not reject older plans solely for missing this section.
6. **Repo-wide completion validation gate** — after all implementation and automated wave validation pass, run the project's full repo-wide validation suite. If the plan has a `## Validation Contract`, run the repo-wide validation command or command set named there. If it does not, use the strongest project-defined aggregate command when available; in this repository that command is:
   ```bash
   make check
   ```
...
7. **Deployment Procedure gate** — after all waves and repo-wide completion validation pass, check whether
...
8. After completion, **archive the plan** only if all implementation, validation, repo-wide tests/lint/format/check commands, and manual/deployment gates pass: set `completed` date in frontmatter, move to `.specs/archive/{slug}/`.
9. Go to **Step 7: Summary**.
