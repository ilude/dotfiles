...
For the Windows Pi package-manager decision (`pnpm` instead of Bun for the global `pi` install), see `pi/README.md`. Do not use `npm` in this repo; prefer `bun`, or `pnpm` where documented/locked. Pi TypeScript validation is pnpm-only and consolidated at `pi/package.json` (single source of truth for runtime deps, typecheck, and Vitest). Do not run `bun add`, `bun install`, `bun run`, or `bun test` for Pi-related TypeScript packages/tests. Use `cd pi && pnpm install --frozen-lockfile && pnpm run typecheck` and `cd pi && pnpm test`. `@earendil-works/*` and `@sinclair/*` are intentionally absent from `pi/package.json` -- they are symlinked from pnpm-global into `pi/node_modules` by `scripts/pi-deps-link-setup` so they always match the installed pi binary version.
