...
Provides bounded process-local management for long-lived Bash commands through `bg_start`, `bg_status`, `bg_list`, and `bg_kill`. `bg_start` passes through the same damage-control shell gate as `bash`; unmanaged shell background operators are rejected because the manager already runs commands asynchronously. The three control tools remain registered but leave the initial provider schema until the first terminal starts or a process survives a session replacement. The `/ps` dashboard shows live state and bounded stdout/stderr; background terminals do not render a separate status widget because the footer already owns process activity status. Natural completion delivers one follow-up result without polling. Managed processes survive `/reload`, `/new`, `/resume`, and `/fork`, and a completion during replacement is delivered to the next active session. Output spills to private capped temporary logs. Pi process exit terminates remaining process trees and removes the logs. Use the scheduler rather than a background terminal for timers or polling waits.
...
Current calls use `subagent_read`, `subagent_write`, or `subagent_coordinate`. Read leaves receive a closed positive read-tool allowlist with no raw shell. Write leaves retain configured mutation tools. Coordinators receive explicit worker, turn, and soft-deadline budgets and return bounded partial results when a budget settles the run. All three interfaces accept a parent-selected `workspaceRoot`; only a root may widen beyond its inherited workspace. `workPaths` and `workBoundary` are advisory trail markers, and overlap is visible but never rejected.

Governed native file tools and recognized recursive `rg`, `grep`, and `find` commands are contained to the effective workspace after canonical and nearest-existing-ancestor checks. Static `cd` targets are checked; dynamic or filesystem-root recursive targets reject. This does not sandbox arbitrary programs launched through shell commands.

`subagent_status` accepts an orchestration ID to group children or an exact run ID to inspect PID, liveness, observable activity, active tools, duration, output age, usage, and a ping-backed watchdog classification. `subagent_control` can target an authorized exact run or tree. A stalled-tool interruption settles the child process tree and resumes its persisted session with an explicit warning that the interrupted tool result, side effects, and unpersisted output are unknown. Completion remains push-based, so status checks are evidence rather than a polling mechanism. Historical `subagent`, `subagent_chain`, `subagent_continue`, `subagent_fanout`, and `subagent_workflow` calls remain executable through one hidden compatibility adapter for resumed sessions.
...
The hidden compatibility `subagent_workflow` retains its historical typed map, retry, verify, and reduce contract for resumed sessions. New work uses the role-specific interfaces. Advisory path markers do not grant mutation authority and need not be disjoint.
...
A root owns durable task creation, state transitions, validation, and closure. Each current subagent item may carry an existing running task ID for correlation. If exactly one eligible running root task exists in the effective workspace, the runtime may link it unambiguously; invalid or ambiguous links return bounded choices. Children never create or transition task records. Disposable delegation remains task-free.
