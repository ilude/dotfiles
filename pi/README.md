...
Interactive zsh and PowerShell make bare `pi` invoke the `pp` profile launcher. New and resumed default-profile sessions record that active profile once as a metadata-only `session-profile` custom entry in the session JSONL; it is excluded from model context.

- `pi` and `pp` use `pi/profiles/default/` with the same startup preflight.
...
The default profile provides Markdown-defined `subagent` delegation with frozen tool and delegation authority, retained conversations, origin-scoped results, Team Lead roles, and explicitly requested councils. Children use native Herdr panes by default inside Herdr and RPC elsewhere; direct user intervention has explicit handback. Parent and child exchanges use native queued steering by default, with explicit immediate redirection and question-answer correlation; a successful question leaves a clear waiting state instead of a polling tool. Automatic outcomes arrive during work without acknowledgement chatter, and terminating tool errors retain their actual provenance. Ordinary chat changes preserve child ownership. Operators do not run `/reload` while subagents are active; the existing settled-only reload ends settled ownership without active-child migration or recovery. Trusted project definitions may override profile roles without unsafe fallback when invalid. Focused automated and runtime checks passed, followed by model-backed and attached-client acceptance after `/reload` on Herdr 0.9.0. No unsupported active-child migration was built. See [definitions, tools, lifecycle, and limitations](profiles/default/docs/subagents.md).
...
- `/plans` opens a framed, temporary picker for direct-child implementation plans, with a highlighted selection, aligned status/task counts, and a scrolling list labeled and sorted by directory stub. The selected preview shows the plan title, `.specs/<stub>/` path, and summary so filenames do not have to be inferred from descriptions. Enter opens wrapped, scrollable details with `o` Open in VS Code, `c` Copy command (`/do-it .specs/<stub>/plan.md`), `r` Run here, `d` Run in new tab, and `a` Archive. Copy uses Pi's cross-platform clipboard helper without executing anything. Run here closes the picker and submits the command with native prompt-template expansion, starting immediately when idle or queuing behind current work. Run in new tab immediately shows Launching in the current picker, ignores repeat input while the asynchronous request is pending, explicitly focuses the returned Herdr tab, and dismisses the picker on success without opening Details. It has no non-Herdr terminal fallback. Failure keeps the original view and selection; clearly prelaunch failures allow retry, while uncertain or already-created results remain protected against relaunch. A shared, profile-local execution reservation shows live `launching`, `running`, `waiting`, `blocked`, or `unknown` status and the owning tab/process without rewriting the plan's frontmatter. Both `r` and `d` are disabled while that exact canonical plan path is owned, including across fresh pickers and other default-profile Pi processes. The picker refreshes once per second and execution atomically reserves the plan before queueing or creating a tab. Waiting/blocked plans remain protected; completed settled work, session replacement/exit, or a confirmed dead owner releases the reservation. Reload retains ownership. Tracking covers `/plans` starts and observed explicit `/do-it .specs/<stub>/plan.md` inputs, not implicit selectors, executions predating this feature, other profiles, or separate worktree copies. Reload before starting tracked work; an existing tab title alone is not evidence of execution. All five action shortcuts work on the selected plan in both Browse and Details; Enter is optional inspection, not an action gate. A high-contrast browse footer lists the available actions; controls take priority over preview text and metadata in cramped terminals. Esc returns to the list, and `q` closes either view without leaving the picker in the transcript. Archive eligibility checks and confirmation remain required. The picker requires interactive terminal mode.
