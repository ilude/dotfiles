...
def test_replay_reports_shell_normalization_and_failure_survival(tmp_path):
    records = [
        {
            "argv": ["cd", "repo", "&&", "git", "status"],
            "exit_code": 0,
            "bytes_before": 500,
            "bytes_after": 500,
            "rule_id": None,
            "reduction_applied": False,
            "stdout_sample": "WARNING: fixture warning\n" + "x\n" * 200,
            "stderr_sample": "",
        }
    ]
    corpus = tmp_path / "shell-corpus.jsonl"
    _write_jsonl(corpus, records)

    result = _run_evaluate("--corpus", str(corpus), "--replay-rules")

    assert result.returncode == 0
    summary = json.loads(next(line for line in result.stdout.splitlines() if line.startswith("{")))
    assert summary["rule_match_rate"] == 1.0
    assert summary["newly_matched"] == 1
    assert summary["failure_survival_failures"] == 0
...
def test_gates_pass(tmp_path):
    ...
    corpus = _make_corpus(
        tmp_path, n_reduced=90, n_passthrough=10, bytes_before=1000, bytes_after_reduced=400
    )
...
def test_gates_fail_reduction(tmp_path):
    ...
    corpus = _make_corpus(
        tmp_path, n_reduced=80, n_passthrough=20, bytes_before=1000, bytes_after_reduced=920
    )
...
def test_no_labeled_skips_fp_gate_reduction_fails(tmp_path):
    ...
    corpus = _make_corpus(
        tmp_path, n_reduced=80, n_passthrough=20, bytes_before=1000, bytes_after_reduced=950
    )
...
def test_passthrough_rate_computed_correctly(tmp_path):
    ...
    corpus = _make_corpus(
        tmp_path, n_reduced=70, n_passthrough=30, bytes_before=1000, bytes_after_reduced=500
    )
    result = _run_evaluate("--corpus", str(corpus))
