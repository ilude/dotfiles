...
import { bedrockModelCost } from "./pricing.js";
export interface BedrockModelMetadata { id: string; kind: "foundation" | "inference-profile"; inferenceProfileName?: string; modelName?: string; }
...
export function resolveBedrockMantleTarget(
	env: BedrockMantleEnvironment = process.env as BedrockMantleEnvironment,
): BedrockMantleTarget {
	const region =
...
export function createBedrockRequestTokenProvider(
	target: BedrockMantleTarget,
	baseEnv: BedrockMantleEnvironment = process.env as BedrockMantleEnvironment,
	factory: TokenProviderFactory = getTokenProvider,
...
function loadBedrockRuntimeMetadata(): BedrockModelMetadata[] {
	return getBuiltinModels("amazon-bedrock")
		.filter((model) => model.id.startsWith("us.anthropic.claude-"))
		.map((model) => ({ id: model.id, kind: "inference-profile" as const, modelName: model.name }));
}
