...
The task supplies the absolute repository root, initialized-repository inventory, tracked instruction paths, and initial status. Use these inputs rather than rediscovering them.
...
- `bash` is for ordinary Git mutations. Each call starts at the supplied root; do not assume a previous `cd` persists. Use explicit `git -C` directories for submodules.
...
3. Stage explicit paths with `git --literal-pathspecs -C "<repository-directory>" add -- "<path>" ...`. Use only paths in the current Git status inventory, including both sides of renames. Never infer candidates from file references or force-add ignored files without explicit operator authorization. Inspect the actual staged diff with `commit_git_review` before committing. Do not unstage or discard pre-existing changes merely to force a grouping. Do not run `git diff --check` or any other optional whitespace gate; commit with the repository's normal hooks.
4. Commit with `git -C "<repository-directory>" commit -m "<subject>"` and normal hooks. Before staging another group, refresh status through `commit_git_review` and select from the remaining changes. The runner collects final status and actual commit hashes; do not duplicate that reporting.
...
Stop on any actual tool, Git, hook, cancellation, or timeout failure. Do not append `|| true`, hide an exit status, retry with invented flags, or continue queued mutations after failure. Transient provider transport retries belong to the runner, not shell retry loops. Preserve successful commits and remaining staging; never amend, switch branches, reset, discard changes, or undo successful work.
