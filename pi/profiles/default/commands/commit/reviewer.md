...
3. Stage explicit paths with `git --literal-pathspecs -C "<repository-directory>" add -- "<path>" ...`. Include both relevant old and new paths for moves. Inspect the actual staged diff with `commit_git_review` before committing. Do not unstage or discard pre-existing changes merely to force a grouping.
