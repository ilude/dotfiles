...
- **Proposed remediation:** Tighten the project write-back and project-knowledge wording around that implemented-or-accepted boundary, then classify and relocate existing intent/history material rather than adding more caveats. Preserve current operating procedures such as CAC seed authoring in `docs/`. No instruction change or repository cleanup has been approved yet.
- **Status:** Discussion pending; do not treat the proposal as authorization to edit project guidance or relocate/delete files.
