...
- **Feedback:** The orchestrator read a council's unresolved list aloud with generic summaries, omitted why items existed and what current evidence established, and repeatedly asked the operator to decide matters that repository evidence could settle. It then proposed changing reconstructed UserSearch source from a proven production WAR to satisfy a newly added synthetic test before checking provenance, production behavior, fixture realism, or identity persistence.
- **Finding:** One-at-a-time presentation does not make an uninvestigated finding decision-ready. The orchestrator must trace origin, date, current relevance, authoritative owner, consumer behavior, provenance, and realistic test inputs first. It should remove, consolidate, correct, or investigate findings directly when evidence determines the disposition, and ask only for a remaining choice that changes behavior, scope, or acceptance. Technical proposals must name the component, paths, mechanism, tests, and evidence boundary rather than use generic phrases such as “update UserSearch.” Existing investigation and proportionality guidance already required this; the primary failure was adherence.
- **Project correction:** The UserSearch issue is now recorded at `.pi/agent-notes/eisa-mps-integration-issues.md` as an open test-fixture-versus-KMIS-persistence investigation. The production-compatible reconstructed implementation is not presumed defective.
- **Status:** Feedback and project evidence recorded. No instruction change authorized.

## AIF-090 - Review findings belong in the orchestrator synthesis path

- **Reference:** Operator review of another Pi orchestrator's read-only synthesis handoff, 2026-09-24.
- **Feedback:** A reviewer should return findings through its normal result, not require filesystem write authority. The orchestrator should correlate and consolidate reviewer and other agent output, then write the requested artifact itself or dispatch a narrowly scoped writer only when a separate artifact is actually required.
- **Finding:** Session `01a0d4c4-952a-7513-a435-2adbbf7e87e6` shows the orchestrator explicitly assigned a reviewer to write the report even though the reviewer role permits shell only for inspection/checks and has no edit/write tools. The reviewer returned a valid synthesis and correctly reported the authority mismatch; the orchestrator then launched a developer to independently inspect and synthesize again. Read-only limits source and artifact mutation, not the reviewer's ability to report findings in its completion message. The always-visible role catalog described reviewer only as “Independent evidence-based review,” while the read-only capability distinction and parent-owned integration statement existed in conditional documentation rather than at the dispatch decision. A dedicated findings-output extension would duplicate the existing result channel and weaken the meaning of a read-only role unless there is a separately established transport or durability defect.
- **Recommendation:** At the existing role-selection surface, make the reviewer description identify it as read-only and make parent ownership of cross-agent synthesis and integration explicit. When a requested artifact must be written, either keep synthesis parent-owned or assign one developer the combined synthesis-and-artifact outcome from the start; do not assign filesystem output to a reviewer or repeat an already completed synthesis merely to materialize it.
- **Status:** Session review complete. No instruction or extension change authorized.
...
- **Observed:** During dev seed rollout, the assistant added one-time deployment evidence to architecture and seed docs, then replaced it with repetitive generic caveats. The operator corrected both. A later uncommitted architecture edit added a desired database lifecycle explicitly qualified as not current, followed by a list of implementation gaps. Recent history also contains several `docs: record ... validation` commits. Those are concrete recurrences: desired future state and investigation/change evidence continued going into `docs/` instead of agent notes, task reports, artifacts, or established history. By contrast, the committed CAC seed authoring procedure is current usage guidance and belongs in developer docs.
- **Finding:** Existing placement guidance names the right homes but still permits broad “contracts and rationale” language, which can be used to justify writing proposed contracts and gap inventories into architecture docs. This is both an adherence recurrence and a boundary that could be made more observable: developer docs describe implemented current behavior or an explicitly accepted durable contract; proposed/required future state, gap inventories, investigation evidence, and per-change validation history default to `.pi/agent-notes/` or task artifacts until implemented or formally accepted.
- **Remediation:** With operator approval, tightened project write-back and project-knowledge wording around the implemented-or-explicitly-accepted boundary and made agent notes the default when placement is uncertain. Relocated the active database lifecycle intent, gap inventory, and dated validation evidence into `.pi/agent-notes/database-lifecycle-gaps.md`; retained current mechanisms in architecture and current CAC seed authoring in developer docs.
- **Status:** Implemented locally. Broader historical cleanup was not performed; future adherence remains unverified.
...
## AIF-091 - A named task is still too large when it bundles independent durability seams

- **Reference:** Iris durability assignment in default sessions `01a0d4e4-c99a-7500-aada-813b6db55846`, `01a0d4e7-ae42-74d9-949d-9fb978c37d66`, `01a0d4e7-e223-77e9-8c37-7e892135bbc3`, and `01a0d4ec-c9be-723e-96b7-d30060f9c741`, reviewed over `[2026-09-24T19:32:40Z,2026-09-24T20:49:16Z)`.
- **Feedback:** The operator identified both assignment breadth and use of a smaller implementation model as likely contributors and requested a decomposition review that includes coordinator sizing and steering, not just child quality.
- **Facts:** The Sol-high Strategist made one “B” assignment cover request persistence, restart recovery, claim leases and terminal fencing, one atomic terminal/outbox transaction, callback and notification retry/idempotency, additive migration, seven configuration settings, observability contracts, lifecycle handoff, and focused tests across at least 13 changed or new task-owned paths. The Astra-high coordinator accepted that grouping and routed it to `gpt-6-luna` at xhigh. Five follow-up messages reached Iris in the first 7 minutes 23 seconds after launch; two relayed unrelated shared-worktree ownership state and three clarified downstream contracts or acceptance details that would have been cheaper to freeze before implementation. Iris was not inactive: within the fixed interval it made 111 tool calls, reported a candidate contract at 19:45Z, produced the implementation and migration evidence, and reported a stable contract at 20:49Z. Its 20:41Z focused run passed 56 tests, although later edits made a final rerun appropriate.
- **Assessment:** Oversizing is established independently of elapsed time or final quality. The grouping conflicts with AIF-083's existing rule that one named task is an upper boundary, because these are separately provable seams with explicit dependencies. A model-fit concern is plausible, but causality is not established: there was no same-assignment comparison under Sol/Astra, and Luna xhigh ultimately produced passing focused checks. More reasoning effort did not turn Luna into a stronger model class. The routing failure began with Strategist and coordinator decomposition, not with Iris alone.
- **Better decomposition:** Freeze the storage/state/event method contracts first. Then (1) give one Sol-high owner persistence, migration, and the complete terminal-plus-outbox transaction across `storage.ts`, `db.ts`, migration/bootstrap SQL, durability data types, and their tests; (2) in parallel after that freeze, give a separate owner `pipeline.ts` claim-token stage fencing and stage/provider event emission with a new disjoint test file; (3) after persistence lands, give one owner `jobs.ts` restart recovery and lease/terminal races; (4) transfer `jobs.ts` ownership only after that result to a delivery owner for callback/notification pumping, idempotency, retry config, and lifecycle wiring in `config.ts`/`vault-service.ts`; then freeze the completed B contract before D. Do not split the atomic database transaction across writers. Use Sol high for the coupled transaction and state-machine assignments; reserve Luna for bounded leaves after contracts are fixed unless comparable outcomes support broader routing.
