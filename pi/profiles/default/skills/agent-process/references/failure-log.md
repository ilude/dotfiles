...
- **Observed:** The Team Lead reported blocked because its own tool catalog lacks shell/write and claimed that developers/validators inherit that ceiling. It cited its restricted `tool_search` returning no shell tools and Strategist agreement, not a rejected developer/validator launch. Its process settled and pane closed without reported project modifications.
- **Finding:** `lib/subagents/runtime.ts` resolves permitted delegates from their own catalog definitions; `lib/subagents/launch.ts` uses that definition's tools in launch arguments and frozen authority. Developer has bash/edit/write; validator has bash. The Team Lead's lack of these tools is intentional coordination-only authority, not a delegation blocker. Restricted tool discovery describes only the caller.
- **Classification:** Agent interpretation/output-quality failure, not an established tool-authority mechanism defect. Another agent's agreement did not verify the premise.
- **Recovery:** Parent verified source and role definitions, then redispatched the same layout/naming scope with the concrete authority correction and asynchronous-control workaround. No authority widening or runtime change.
- **Status:** Corrected assignment dispatched as `7c689b88-22a5-45b1-9c9f-4771a51c250b`; implementation outcome pending at original review. No global prompt change approved. See TCA-008.
- **Operator follow-up:** The operator identified this refusal as another subagent coordination improvement on 2026-09-17. Exact transcript recheck shows the lead introduced the false inheritance premise in its Strategist assignment (`f2e8776f`, 16:01:54Z); Strategist repeated it, then local-only tool discovery returned an empty result (`d52677ac`) and the lead reported blocked (`f7e12e43`, 16:02:57Z). This was not independent confirmation. Recommend making personal tool authority versus permitted delegate capabilities explicit in the composed coordinator guidance/catalog and requiring capability-based dispatch rather than inferred team-wide inability. Preserve the lead's existing tool restriction; do not widen authority. No instruction changes made.
- **Agreed direction:** The operator clarified that the remedy belongs in the handoff prompt the Team Lead starts with: explain its own coordination tools versus the separate role-defined tools of permitted delegates. This is startup-context clarification, not permission expansion or a new dispatch gate. The operator refined the selected wording to explain both discovery and inheritance: "Your tool discovery lists only your own tools, your subagents do not inherit your tool restrictions, they will have tools needed to do their work." Keep the handoff clarification this concise, with role-defined tools as the precise capability boundary. The longer proposed explanation is unnecessary.
- **Implementation (2026-09-17):** At the operator's request, `childSystemPrompt` now adds only the concise discovery/inheritance clarification to the Team Lead startup authority handoff. Delegate capabilities remain role-defined; permissions and messaging behavior are unchanged. Default-profile loader, launch-prompt, and guidance tests passed (22 tests), as did typecheck. Rendered Team Lead guidance adds 149 fixed bytes; other role prompts are unchanged. The change begins after the existing authority restriction; dynamic native base/context/tool sections remain outside the static render. No live-model or provider-cache behavior was measured.

## APR-057 - Coordinator messages implicitly wait for a background worker

- **Reference:** Screenshot follow-up, 2026-09-17; default root `01a0ad5f-c353-7153-a106-a6d55654b821`, Team Lead `01a0afcc-d9b2-7162-a92f-845f14c4fb61`. Related review TCA-007 and feedback AIF-069.
- **Observed:** Clara received Nora's full helper result at 14:44:13.391Z (`091ba4f7`), then answered Iris's prerequisite question using `subagent_control message` plus `replyTo` at 14:44:18.616Z (`6085f0b8`). That tool returned only at 14:54:53.601Z (`558528f5`), after Iris completed. Root cleanup steering sent at 14:47:36.241Z (`60c1729d`) entered Clara's transcript at 14:54:58.654Z (`63ce7dfe`). Clara finished Nora at 14:55:04.691Z and pane cleanup completed at 14:55:05.984Z (`4be38990`).
- **Source:** `extensions/subagent-child.ts:68-72` adds `waitForChild` to every coordinator message/answer unless that control call sets `background:true`; `waitForChild` polls every 250 ms while the child runs. The tool description advertises queued steering but does not explain the implicit join. Root `extensions/subagents.ts:93-102` dispatches and returns a snapshot without that join. `VisibleChild.command` only enqueues prompts (`lib/subagents/visible.ts:135-138`), so the long wait is in the coordinator wrapper, not visible transport. The wait predates the recent mailbox change, as shown by Git history.
- **Retention:** All three developers were launched with `retain:true`. Nora correctly completed and remained available. The caller already had her result; this was not a missing notification. Useful retention is allowed, but the implicit join prevented Clara from considering cleanup or acting on steering while Iris worked.
- **Additional workflow finding:** Clara imposed Steward consultation after an already-corrected formatting-only check, and Steward answered in permission terms. Generated Team Lead guidance lacks the caller's evidence-proved-correction exception. Broadening every check into an approval step contributed a needless question/answer cycle.
...
- **Reported:** 2026-09-17, while investigating unusual end-of-session subagent behavior in default-profile session `01a0acad-f5d2-738a-942f-f5858de7a2c4`.
- **Observed:** The Team Lead launched Strategist Maya with `retain:true`. Maya delivered its plan through the foreground wait but remained alive, contrary to the one-shot blocking Strategist contract. Direct operator input in the improperly retained pane then started an unintended second turn. Separately, after the root redirected a Team Lead that was coordinating recovery, the Team Lead asked its parent a second question and then its visible process exited with code 0 before the answer arrived. The root's answer was rejected because the assignment was already cancelled. Team Lead shutdown also produced contradictory or blank `exit`-turn reports from retained Explorer and Reviewer children, despite their substantive work already being available. A Steward subsequently reported `pane_not_found` during cleanup after returning useful analysis.
- **Evidence:** Exact root-session SQL covered all 208 records then present over the session's full initial interval, a later literal search covered all 218 records after the transcript grew, lineage enumerated the relevant descendants, and exact SQL staged four late descendant transcripts. The root contains no end-sequence `/reload` evidence. Runtime source says reload refuses to retire while child conversations, processes, or retained children remain active rather than migrating them.
- **Classification:** Tool-contract and caller failure for accepting `retain:true` on a Strategist; agent/tool lifecycle failure for the cancelled nested question and inconsistent retained-child settlement; expected record pruning for later `Unknown child` responses. A `/reload` explanation is unproved.
- **Assessment:** The nested failure occurred on the recently changed asynchronous parent-question path, making a regression plausible, but one earlier question succeeded in the same Team Lead and existing focused checks passed. The evidence establishes a real one-session defect, not its exact implementation cause or recurrence.
- **Decision:** Correct the prior interpretation: Strategists must be foreground, blocking, and non-retained. Record the clarified invariant and treat runtime enforcement as the preferred fix rather than relying on caller adherence. Keep the existing rule to let active children settle before reload. Separately investigate or reproduce the redirect-then-question lifecycle and retained-child shutdown before attributing that incident to the mailbox implementation.
- **Status:** Strategist retention defect fixed in both launch paths; fourteen focused tests, default typecheck, and runtime smoke passed. The separate nested-question lifecycle issue remains open.
...
- **Reference:** EISA dev OAuth identity migration, pipeline 8491 and operator certificate-login screenshot.
- **Observed:** The assistant declared deployment accepted after verifying that the login endpoint emitted the new client ID and KMIS redirected it into X.509 authentication. The operator's real login then completed certificate authorization but failed during authorization-code redemption because the token endpoint returned 401.
- **Finding:** The acceptance claim exceeded the evidence. A successful authorization redirect proved client discovery and callback acceptance, not the complete login flow or valid client authentication at the token endpoint. The later rollout-order explanation was not just unverified: the displayed timing showed ExternalSecret last sync at 112 seconds and pods at 101 seconds, so the sync preceded pod creation by about 11 seconds. The assistant incorrectly reversed that ordering and implied a completed deployment had not recreated pods even though the output showed fresh replacement pods.
- **Remediation:** Retract completion for the login flow and the stale-pod explanation. Report authorization and token redemption as separate stages. Read relative ages in the correct direction, reconcile hypotheses against displayed rollout evidence, and do not restart workloads as a causal fix without a discriminating check.
- **Related:** APR-001/APR-006 (premature completion), deployment simplification feedback above (insufficient end-to-end evidence).
- **Status:** Feedback recorded. MVC was restarted after secret synchronization, but end-to-end operator acceptance remains pending.
- **Recurrence:** During the follow-up fix, the first implementation replaced last week's administrator-readable DCR proof with client-credential DCR proof instead of preserving both. This repeated the same partial-proof mistake across the APIM and EISA consumers. Parent review caught it before push and returned it for correction, but the operator again had to identify the broader recurrence.
