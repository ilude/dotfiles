...
4. When a change is warranted, propose the smallest useful addition, replacement, consolidation, deletion, or relocation at the owning scope. Before adding model-visible text, compare it with related active instructions and ordinary model knowledge. Keep local constraints, operator policy, and evidence-backed corrections explicit; omit or consolidate generic and duplicate guidance. More instructions are not automatically better: they consume task context and can introduce conflicts or obscure important guidance. Prefer replacing or consolidating existing wording when it preserves the intended behavior. Explain its expected behavioral effect, supporting evidence versus assumptions, and what later evidence would justify revising it. Match investigation effort to the actual decision; do not require scores, citations for every tentative thought, fixed thresholds, experiments, or another review process.
