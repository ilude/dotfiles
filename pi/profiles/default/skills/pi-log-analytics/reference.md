...
Optional discovery fields: `cwd` (one exact checkout after platform path normalization), `repository` (a readable Git checkout plus its linked worktrees), `sessionIds` (native IDs), `maxRows` (1-1000, default 100), and `cursor` (the preceding `nextCursor`, with identical filters). `cwd` and `repository` are mutually exclusive. Results contain `profiles`, `sessions`, `truncated`, `nextCursor`, and coverage notes. Each session has `ref: { profile, sessionId, fileKey }`, nullable `cwd` and `created`, file `modified` time, and `bytes`. Modified time is filesystem metadata, not the last event timestamp. Listing is best-effort under concurrent changes, not a snapshot.
...
Search interval is fixed `[since,until)`, based on normalized outer/native-nested record timestamps. Search accepts the same mutually exclusive `cwd` and `repository` location scopes as discovery. Equivalent Windows separator, case and trailing-separator spellings compare as the same checkout; repository scope resolves linked worktrees through Git metadata. Cursor scope binds the normalized location and interval, so continuation cannot silently broaden either. Search text is literal and examines message strings/text blocks only. Set `filters.subagentBlocking` to `true` for foreground launches and explicit waits, or `false` for background launches and nonblocking controls. Matching assistant records include `blockingDecisions` with tool, agent/action, effective background mode, reason, reason source (`model`, `role-contract`, `missing`, or null for nonblocking calls), and the active subagent extension version (`null` for older unversioned history). With `subagentBlocking` set, `subagentExtensionVersion` restricts results to one recorded runtime version. With `subagentBlocking: true`, `subagentBlockingReasonSources` can restrict results to selected blocking sources. Use `["model", "missing"]` for behavioral reviews that exclude role-mandated Strategists; omitting the source filter preserves complete blocking searches. This targeted extraction does not make other tool arguments text-searchable. `follow_up` revalidates the file and identity, streams to the exact occurrence without DuckDB or whole-file staging, and returns only bounded adjacent context; expansion does not fetch again.
...
- Search cursors live in the session-owned analytics worker and retain at most 16 MiB of measured serialized metadata across active cursors, evicting the oldest first. They bind normalized scope/filters, selected-file identity markers and captured byte horizons. They are released when complete and expire on worker exit or reload. Appends beyond a captured horizon belong to a fresh scan; replacement/truncation stops continuation with an inventory-change result. Do not treat an empty early page as exhaustive.
- When execution is omitted, selections without an exact-session scope use large mode; exact-session selections below 256 MiB use standard, while selections at or above 256 MiB use large. This conservative threshold keeps the measured broad default-profile selection (about 1.58 GB) on the resilient path; it is a policy boundary, not a universal performance crossover. Explicit `standard` and `large` always override automatic selection.
- Both modes use two threads, a `2GB` DuckDB memory limit, and a bounded 8 GiB owned-disk budget configured by `PI_ANALYTICS_LARGE_DISK_BUDGET_BYTES`. Standard uses an in-memory database with owned spill; large uses disk-backed staging and spill. Each invocation stores its artifacts under `pi/profiles/default/.analytics-state/log-analytics-tmp/`, not the OS temp directory. Its unique owned directory is cleaned after success, error, or cancellation; cleanup failures name the exact remnant. Disk-budget failures apply to either mode.
- Costs report requested and effective execution, selected bytes, selection reason, and bytes/resource usage. A standard resource failure recommends an explicit retry with `execution:"large"`; the tool does not retry silently. Spill does not eliminate every OOM: complex blocking operators and some aggregates may still exceed memory.
- Returned SQL rows remain at most 1,000 and 256 KiB for the encoded rows array. `truncated` means output hit those bounds, not that input was silently omitted.

Query results return `columns`, `rows`, `truncated`, `cost`, `profiles`, `sources`, and coverage notes. DuckDB serializes BIGINT values as decimal strings and JSON values as JSON text. `cost` reports `requestedExecution`, effective `execution`, `selectedBytes`, `selectionReason`, `filesScanned`, `bytesScanned`, `discoveryMs`, `stagingMs`, `queryMs`, and resource usage including disk high-water and budget. Staging includes queue wait, instance setup and materialization; end-to-end latency also includes lazy loading and cleanup.
...
Filesystem and native DuckDB operations run in a session-owned child process. A worker crash is reported to Pi with its exit code or signal and bounded stderr; session shutdown closes the worker. The disposable metadata cache is `.analytics-state/metadata.json` beneath the default runtime root. One cache instance is shared during a search. It stores only file markers, validated session header metadata and event ranges learned during requested reads. It contains no transcript content or permanent SQL projection. Missing, corrupt, stale or unwritable cache state falls back to authoritative discovery and cannot exclude a file.
