...
- Submit commands only to verified idle Bash/PowerShell shells. Unknown foreground processes or cwd are not safe command targets. No agent delegation.
...
- Interrupt gracefully, then inspect. Close only verified panes created by this Pi session, with explicit tool confirmation. Ownership is intentionally forgotten on session replacement/reload; do not invent ownership to close older panes.
