...
Command tools are registered but inactive initially. A command submission prepares its schemas additively before delivery, so Pi's next response has the tools even when the command is steered into a busy session. Preparation is availability only, not authority. The hidden prompt message carries a process-local invocation ID; authority is selected only when that message is delivered through native `deliverAs: "steer"` and `triggerTurn: true`. Each tool call is then bound to that delivered invocation and its immutable options. A later `/commit push` cannot change an earlier bare `/commit` call, and arbitrary or restored message details cannot create authority. Tools remain available through retries, compaction, and ordinary steering, then are removed at settlement or session shutdown. Built-in tools and other extensions' tools are preserved. Execution also checks command ownership. This controls availability, not a security sandbox: ordinary shell tools still have their normal permissions. `/commit` handles ignore-file decisions inside its running tool through Pi's UI. Answering resumes the same Luna task, with its active-work timer paused during the question.
