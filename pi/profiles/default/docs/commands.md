...
`extensions/commands.ts` runs the explicit registry in `commands/index.ts`. Everything belongs to this profile; no other profiles or project command directories are scanned. Legacy is unchanged.
