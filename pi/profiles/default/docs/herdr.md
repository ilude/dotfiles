...
The picker refreshes once per second and shows `launching`, `running`, `waiting`, `blocked`, or `unknown` instead of the saved status while a reservation exists. The owning tab/process appears in the selected preview and Details; Details retains the saved plan status. `r` and `d` are disabled in both views for every owned state. Copy, open, navigation, and existing archive checks remain available.

Pi lifecycle events, not tab labels or generic Herdr agent badges, update execution state. Queued work remains waiting until its actual user message is delivered, including follow-ups within the same agent loop. Blocking prompts report blocked. An incomplete settled run stays waiting and reserved; completion (including a completed archive) releases only after the agent settles. `/new`, `/resume`, `/fork`, and normal exit release owned work; `/reload` preserves it. Closing/crashing the owner permits reclamation after its PID is confirmed dead. An unresolved launch reservation is kept conservatively while its initiating process remains alive.
