...
Safe prelaunch failures preserve the view and selection and allow retry. A timeout, uncertain response, or failure after tab creation warns that the launch may exist. The shared reservation remains in force across fresh `/plans` invocations; do not relaunch an uncertain request simply by reopening the picker. A late child can adopt the same reservation. If it never starts, inspect the destination before closing the initiating Pi process to retire its unresolved reservation. Tab creation/focus alone is not a readiness acknowledgment from Pi or proof that the model has begun executing the plan.

### Live plan ownership

Default-profile Pi processes share small gitignored execution records under `getAgentDir()/plan-runs/`, keyed by canonical plan file path. Atomic claim operations prevent two already-open pickers from concurrently starting the same plan. A random token identifies the reservation during the parent-to-child handoff; PID liveness cleans up confirmed dead owners. This is local runtime state, not a daemon, session-history scan, or frontmatter edit. Storage or ownership uncertainty disables execution rather than treating the plan as available.

The picker refreshes once per second and shows `launching`, `running`, `waiting`, `blocked`, or `unknown` instead of the saved status while a reservation exists. The owning tab/process appears in the selected preview and Details; Details retains the saved plan status. `r` and `d` remain available: an explicit action replaces the existing ownership record with a new token. Copy, open, navigation, and existing archive checks remain available.

Pi lifecycle events, not tab labels or generic Herdr agent badges, update execution state. Queued work remains waiting until its actual user message is delivered, including follow-ups within the same agent loop. Blocking prompts report blocked. Once delivered work's agent loop settles idle, execution tracking is released regardless of saved plan markdown status; the record describes observable activity, not semantic plan completion. `/new`, `/resume`, `/fork`, and normal exit release owned work; `/reload` preserves it. Closing/crashing the owner permits reclamation after its PID is confirmed dead. An unresolved launch reservation is kept conservatively while its initiating process remains alive.
