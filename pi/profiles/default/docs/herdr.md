...
Herdr's generated `extensions/herdr-agent-state.ts` is checked in at integration version 9 from Herdr 0.9.1. Upstream now handles Windows session paths with explicit POSIX and Windows absolute-path checks, so the previous repository correction is no longer needed. Pi's original `reload` lifecycle reason is retained. To refresh the generated file after a Herdr update, run `herdr integration install pi` with `PI_CODING_AGENT_DIR` explicitly pointing to default and inspect the diff. Do not overwrite concurrent changes. The repository prompt bridge is separate and must not be copied into generated code. Windows installation resolves the latest official stable Herdr release with 0.9.0 as its fallback minimum. The layout remains compatible with 0.8.x APIs; upgrading a client does not update an already-running server until that session is restarted.
