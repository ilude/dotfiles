...
`subagent` uses native TUI children by default inside Herdr, with an explicit headless override. It verifies the configured `local.pi` bootstrap belongs to the active profile's repository and owns only returned pane IDs. The current 0.8.2 layout evidence preserves focus for no-focus operations after creation, but the required initial swap still steals focus and the 5+ child second row remains physically blocked; the isolated diagnostic probe expects that theft and is not acceptance. Children have no general Herdr control tools. Ordinary parent chat changes preserve separate subagent ownership, but explicit `/reload` is a subagent lifecycle boundary that ends all subagent runtime, conversation, and process ownership, including idle retained children. Ordinary process-tool ownership still expires as documented above.
