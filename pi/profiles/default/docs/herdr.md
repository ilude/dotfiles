...
- Interrupt/close is limited to panes created by this Pi session; closing additionally requires `confirm: true`. Move may target an existing pane because operator-directed layout organization is its purpose, but Pi's own pane is protected. Session replacement or reload forgets ownership without closing services. Older panes can still be read or moved; their cleanup requires explicit operator action rather than invented ownership.
