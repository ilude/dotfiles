...
Active command policy IDs describe operations rather than migration order. Details and judge evidence carry those same identities; path checks also include the actual protection pattern. Historical IDs remain only in migration evidence and the task disposition table.

Luna receives user text and visible assistant response text from the native active session branch, the pending tool's name, input and execution cwd, and applicable rules. The full user/assistant text is sent without a message-count or byte cap. Only a provider-reported context-window overflow triggers trimming: older messages are removed in progressively smaller tails until the request fits, with the omitted count disclosed in the judge input. The pending call and applicable rules are never shortened. Context recovery shares the existing review deadline; unrelated provider errors do not trigger trimming. Resume, reload and tree navigation use the selected branch rather than a separate tool-history cache. Thinking, images, custom entries, prior tool calls/results, parser-effect inventories, variable observations and sequence dumps are excluded. Sensitive text is redacted before transmission. The short review contract focuses on meaningful unrecoverable harm, not universal proof that routine cleanup is disposable. Deterministic analysis and retained approval boundaries remain local and unchanged; there are no mandatory environment scans or resource ledgers.
...
This makes provider calls using the configured Luna account when available. It checks sampled judgment, not a guarantee for every environment. On 2026-09-11, before the fixed conversation cap was removed, a non-executing replay of the reported temporary evaluation-directory cleanup using native conversation text returned `allow`; the prompt was about 7.6 KB versus 24.5 KB and took 4.8 seconds versus the recorded 11.3 seconds. Meaningful uncommitted-work deletion still required approval and home-directory destruction still hard-blocked. These measurements do not establish a general latency or timeout guarantee. On 2026-09-09 the full 25-case environment corpus passed with 20 real Luna reviews and no mismatches; no submitted action executed. The isolated live scan also passed with two real read-only children, unchanged and cross-worktree reuse, changed-version fallback, and an invocation review. It used task code with the active default profile's existing authentication and normal Herdr-visible child behavior; no production relinking or credential copy was performed. These are sampled acceptance results, not exhaustive safety proof.
