...
Luna receives bounded session-local context: up to 16 direct interactive/RPC inputs and eight successful covered tool observations, each collection limited to 16 KiB and 30 minutes. Observations retain the originating operation and cwd with the output; they remain untrusted, not authorization. Queued input is used only after delivery. Context is redacted before transmission and cleared on session changes, tree navigation, reload, shutdown, or cancellation. No history is imported after reload, so missing consequential environment facts may require a prompt. Static assignments and explicitly supplied execution-boundary values can resolve targets. Relevant unresolved variable names may also carry bounded gate-process observations, clearly labeled as potentially overridden by shell startup, prefixes, spawn hooks, or remote panes. These observations never turn an unresolved target into a statically proven one. There are no mandatory environment scans or resource ledgers.
