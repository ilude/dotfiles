...
A bundled-Pi visible launch/follow-up/completion case also passed historically with `PI_SUBAGENT_UX_LIVE_REAL=1`; that evidence does not establish reload activation, physical attached-client focus, or operator acceptance. The reported stale-owner/reload and focus/layout defects remain the subject of the active follow-up plan. Do not describe the earlier bundled case as a complete live UX pass. Final bounded validation passed 118 tests with 6 skips across the selected files, but no model-backed or attached-client T5 run occurred. Typecheck retains the baseline commit-whitespace TS7016 error; runtime checks passed 335 rules and 8 schemas.
