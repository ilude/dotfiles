...
After review or validation findings arrive, normally consult Steward before assigning or implementing follow-up fixes, while handling obvious bounded corrections directly. Supply the request, agreed checks, findings, and proposed fixes together, and reuse its advice unless findings or the proposed fix change. No findings require no Steward call. Steward compares evidence with agreed scope and can recommend a bounded fix, a user question, deferral, or closeout; it is not an approval gate or a deterministic finding filter. Findings do not automatically authorize new requirements, audits, subsystems, or acceptance checks. Steward defaults to Luna high, allows Luna high or xhigh, and has no automatic larger-model retry. Selecting Sol or Astra for Steward requires a concrete justification and user approval. This restriction is specific to the model selection, not ordinary Luna consultation. Offline prompt and definition checks do not prove that Steward prevents review churn.
