...
Outside explicit reload, same-process chat changes preserve children under the existing origin-scoped lifecycle. A settled-only `/reload` ends the settled subagent runtime before replacement; it does not migrate active children. Background results wait for their originating chat, with journal-event acknowledgement preventing repeat delivery. Results are not delivered into whichever chat happens to be active. There is no durable registry or parent recreation after process exit.
