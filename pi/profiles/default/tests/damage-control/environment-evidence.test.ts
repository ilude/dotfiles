...
describe("bounded process observations", () => {
  it("keeps relevant host values separate from resolved shell targets and hides credentials", async () => {
    ...
    const projection = projectEvidence({ callId: "env", operation: command, operator: [], untrusted: { effects: analysis.effects, matches: [], uncertainties: analysis.uncertainties, variables }, omissions: [] });
