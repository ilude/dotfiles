...
it("registers one management command, records once, normalizes cost, and reports outside context", async () => {
	...
	const pi: any = { registerProvider: (value: any) => provider = value, on: (name: string, fn: any) => hooks.set(name, [...hooks.get(name) ?? [], fn]), registerCommand: (name: string, command: any) => commands.set(name, command) };
