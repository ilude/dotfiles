# Legacy Pi Agent Setup

This guide belongs to `pi/profiles/legacy/`, selected by `pp -p legacy`. It does not describe the default profile. See the [legacy documentation index](docs/README.md) and [cross-profile guide](../../README.md).
...
The local dotfiles install also defaults `PI_CACHE_RETENTION=long` in the installed shell profiles (`zsh`, `bash`, `sh`, and PowerShell) unless you have already set a different value. That prefers extended provider-side prompt caching where Pi supports it (currently documented by Pi as Anthropic 1h and OpenAI 24h for direct API calls). Bedrock Fable 5 and 5.1 are exceptions: the Fable compatibility hook removes the 1-hour TTL so their expensive cache writes use Bedrock's default 5-minute retention. Other Anthropic models retain the global preference. OpenAI and OpenRouter-hosted OpenAI prompt caching are automatic for eligible long prompts; provider-specific `cache_control` markers are only for models/providers that require Anthropic-style caching semantics.
...
### Bedrock Claude cost-aware delegation

When the resolved primary model is Claude Fable or Opus on either `amazon-bedrock` or `bedrock-mantle`, it receives concise guidance to delegate substantial investigation, implementation, validation, and parallel work to the smallest capable Codex subagents. It may instead use normal direct tools when the task is small or delegation would cost more than completing it. The Claude root owns decomposition, high-level decisions, integration, and the final response, and may use a bounded Team Lead package when multiple independent specialists provide a concrete benefit.

Delegated children remain routed through available `openai-codex` models. Explicit models and agent pins must name an available `openai-codex` model; omitted and size-based selections use the existing dynamic size policy. One invalid member rejects the complete batch or workflow before spawn. Delegation uses runtime-generated private output paths, and provider-visible foreground results remain bounded with complete truncated output written to a private temporary artifact. Saved-session continuation remains unavailable for this provider route.

Other primary models retain their normal guidance and routing. Standard tool contracts, damage control, child authority, depth limits, worker limits, and root-owned integration continue to govern direct and delegated work.
...
- The actual-SDK background receipt seam is opt-in and requires `PI_TEST_SDK_ROOT` to point to the maintained SDK source worktree: `cd pi && PI_TEST_SDK_ROOT=/path/to/pi-sdk pnpm exec vitest run --config tests/vitest.receipts.config.ts`.
...
Ponytail-derived capability decisions and their canonical local owners are tracked in [Ponytail capability tracking](docs/upstream/ponytail.md). Run `just ponytail-upstream` to compare its reviewed commit with the current upstream default branch without changing local files.
...
Use `pp` to launch Pi with an isolated global configuration and runtime directory:

```bash
pp
pp --profile legacy
pp -p experimental --model openai-codex/gpt-5.6-sol
```

`pp` uses the repository-owned `~/.dotfiles/pi/profiles/default/` profile when no profile is named. `pp -p legacy` uses `~/.dotfiles/pi/profiles/legacy/`, which owns the existing customized setup after running `scripts/migrate-pi-profiles.ps1` with Pi stopped. Other named profiles are created on first use under `~/.pi/profiles/<name>/`. Each standalone profile keeps its settings, authentication, packages, extensions, skills, sessions, history, and other runtime state separate. `pp` reserves `-p` for profile selection; use Pi's `--print` spelling for print mode, or place Pi's short `-p` after `--`.
...
`log_analytics` is the sole generic Pi JSONL analytics tool. `catalog` lists stable source IDs and same-named DuckDB views; `query` accepts `{ sources, sql, parameters, maxRows }` and passes documented DuckDB SQL to an invocation-local in-memory engine. Each selected view preserves the complete original JSON record in `record` and exposes `_source_file`, `_record_key`, and `_timestamp` plus typed convenience columns. For example: `SELECT _source_file, _record_key, record FROM session_entries WHERE _timestamp >= $start AND json_extract_string(record, '$.message') LIKE $needle ORDER BY _timestamp`. Use CTEs, JSON functions, date predicates, and ordinary DuckDB parameters; do not use filesystem functions or external table functions.

Each query session defaults to a 5,000 ms deadline, 512 MiB of selected input, 2 DuckDB threads, and a 1 GB DuckDB memory ceiling. Override these bounds with `PI_ANALYTICS_TIMEOUT_MS`, `PI_ANALYTICS_MAX_INPUT_BYTES`, `PI_ANALYTICS_THREADS`, and `PI_ANALYTICS_MEMORY_LIMIT`. Input over the configured limit fails with `exceeds bound` before staging. Results stop incrementally at the row and encoded-byte bounds, report `truncated`, and include `cost: { filesScanned, bytesScanned, stagingMs, queryMs }`; there is no persistent analytics database or refresh state.

Active readers use this direct query boundary or typed in-process APIs. Typed report readers retain their existing unbounded-input behavior. `/find-fails`, `/usage`, `/extension-stats`, `/skill-stats`, `/orchestration-stats`, and workflow-friction diagnostics retain their owning command contracts. Correlation is exact or deterministic by default; unique inferred edges are disclosed, opt-in, and never decision authority. Canonical JSONL remains authoritative.
...
Provides bounded process-local management for long-lived Bash commands through `bg_start` and `bg_kill`. `bg_start` passes through the same damage-control shell gate as `bash`; unmanaged shell background operators are rejected because the manager already runs commands asynchronously. Both model tools remain active from session start. The `/ps` dashboard provides operator inspection with live status and bounded output; background terminals do not render a separate status widget because the footer owns process activity status. Natural completion delivers one bounded follow-up through the SDK's acknowledged `sendMessageWithReceipt` API without polling. The required parent session ID and canonical parent `ctx.cwd` are captured before process spawn; new, forked, or switched sessions never receive foreign completions, while the original resumed session may. Completions remain in the manager until an inserted receipt or explicit `bg_kill` consumption. Queue-cleared, aborted, and replacement receipts pause retry until interactive input; rejected delivery retries only at a later lifecycle boundary; uncertain delivery is held and never blindly retried. A missing receipt capability fails before spawn and never falls back to ordinary `sendMessage`. Delivery state survives `/reload`, `/new`, `/resume`, and `/fork` through the existing manager. Output spills to private capped temporary logs. Pi process exit terminates remaining process trees and removes the logs. Use the scheduler rather than a background terminal for timers or polling waits.
...
See [orchestration telemetry](docs/orchestration-telemetry.md) for field schemas, joins, validation order, reader bounds, and report definitions.
...
[Goal Execution](docs/goal-execution-domain.md) for the
...
- Task records and dependency edges are committed atomically in SQLite. Separate Pi processes observe committed state directly without a process cache. Legacy JSON import and rollback export require the quiescent migration workflow documented in [Goal Execution](docs/goal-execution-domain.md).
...
Current calls use `subagent_read`, `subagent_write`, or `subagent_teamlead`. Read subagents receive the fixed `read`, `grep`, `find`, `ls`, `log_analytics`, `web_search`, and `web_fetch` allowlist regardless of agent frontmatter, with no shell, file mutation, or delegation. Write subagents retain configured execution and mutation tools. Use write execution for validator commands or Git diff review without authorizing source edits; read-mode review requires supplied diff or command evidence. Team Leads receive file-reading tools, `log_analytics`, and `subagent_read`/`subagent_write`, plus explicit subagent, turn, and soft-deadline budgets, and return bounded partial results when a budget settles the run. All three interfaces accept `instructions`, `boundaryPaths`, `boundary`, and an `enforcedBoundary`; only a root may widen beyond its inherited boundary. Advisory paths are markers, and overlap is visible but never rejected.
...
[expertise storage and retrieval](docs/expertise-layering.md).
...
[expertise storage and retrieval](docs/expertise-layering.md) for current ownership,
