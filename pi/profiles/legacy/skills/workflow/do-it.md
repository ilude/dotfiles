...
Direct interactive or RPC input is routed to `/do-it` only for an execution verb plus an exact canonical plan path, or `this plan` when one recent canonical plan is uniquely identified. Trusted extension input is not eligible to mint that route. Native argument completion uses a cached observational plan list, refreshed asynchronously on session transitions, plan readiness, and successful archival. Use the root-only `workflow_inspect` tool for current evidence; completion filtering never runs filesystem or Git work.
...
Treat the plan's `Completion Evidence`, requested acceptance, stated invariants, and safety boundaries as the closed execution contract. Plan routing claims, implementation claims, validation claims, integration, and cleanup are separate observations; a checkbox or commit is not proof that behavior was validated. Preserve source requirement identifiers when supplied, but do not invent requirement IDs or additional contracts. Do not silently weaken, strengthen, or reinterpret the contract. If completion evidence is missing or repository evidence cannot resolve competing material interpretations, stop and ask for the product or design decision.
