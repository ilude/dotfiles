...
3. Keep requests bounded. The tool refreshes registered sources internally and enforces a 5,000 ms session deadline, a 512 MiB input limit, 2 DuckDB threads, 1 GB of DuckDB memory, at most 1,000 rows, and 256 KiB of encoded output. Override the resource limits with `PI_ANALYTICS_TIMEOUT_MS`, `PI_ANALYTICS_MAX_INPUT_BYTES`, `PI_ANALYTICS_THREADS`, and `PI_ANALYTICS_MEMORY_LIMIT`. Input over the configured limit fails with `exceeds bound`; cancellation and unknown source IDs also fail explicitly. Query results report `cost` with `filesScanned`, `bytesScanned`, `stagingMs`, and `queryMs`, plus output truncation.
