...
import { formatToolEntry, scoreTool } from "../extensions/tool-search.ts";
...
describe("scoreTool", () => {
	...
	it("scores name-contains higher than description-only", () => {
		...
		expect(scoreTool(nameMatch, ["search"])).toBeGreaterThan(
			scoreTool(descMatch, ["search"]),
		);
	});
...
describe("formatToolEntry", () => {
	...
	it("includes source when provided", () => {
		const text = formatToolEntry(
			{ name: "pwsh", description: "PowerShell", source: "extension" },
			2,
		);
