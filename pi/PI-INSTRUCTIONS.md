...
Use lightweight prose plans for ordinary multi-step work. Task entries are index cards, not handoff documents; use them only for durable dependencies, cross-turn handoff, or background execution. Keep the summary to a one-line deliverable and notes to blockers, dependencies, or acceptance checks; store detailed context in an artifact and reference its path. Never copy conversation summaries, plans, diffs, or investigation narratives into task fields. Record lifecycle changes only when state changes. Start background execution once, retrieve output when needed, and avoid polling task actions in loops.
