...
| Topic | File |
|-------|------|
| React patterns | [react.md](react.md) |
| Next.js patterns | [nextjs.md](nextjs.md) |
...
- Run typecheck and lint when they validate the changed contract: `bun run typecheck` (or `tsc --noEmit`), `bun run lint`, `bun run biome check .`.
- Run tests when relevant to the changed contract: `bun test`, `pnpm test`, `vitest run`.
...
Extract literals to named constants or enums when they represent a repeated or shared domain concept, configuration, or when the requested change needs a named value.
