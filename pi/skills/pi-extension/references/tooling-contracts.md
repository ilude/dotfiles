...
- Reporting: every successful schedule action reports the next active run as a human-readable `Next scheduled run:` line. Use the schedule's explicit timezone when set and the process-local timezone otherwise. End the assistant turn with the exact line from the final schedule tool result; report `none` when no active schedule remains.
