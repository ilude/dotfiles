...
- Bypass eligibility is enforced at the centralized dangerous-command and protected-path seams. Hard blocks, sequence protections, ambiguous or dynamic shell targets, remote/cloud/live infrastructure actions, remote Git mutation, Docker volume deletion, and exfiltration remain protected. A `.env` path is bypassable only after canonical containment under the current `ctx.cwd` succeeds; containment failure and symlink escape retain protection.
