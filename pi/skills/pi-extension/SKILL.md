...
A recovery handoff never authorizes replay, safety bypass, credential disclosure, or changes to ownership, submodules, destructive, or production boundaries. Cancellation, abort, secrets, credentials, permissions, approvals, blocked safety decisions, and production mutations do not trigger automatic recovery turns. For `/do-it`, the model performs archive and commit work in the owned context. The default policy also merges and cleans owned resources; an explicit commit-and-retain plan policy verifies non-merge state and preserves the owned branch, worktree, and ownership record. Canonical persisted plan status controls normal routing, while conflicting repository evidence routes to reconciliation rather than implementation replay.
