...
export function candidateLedgerState(scan: FailureScan, records: readonly Decision[], toolNames?: ReadonlySet<string>, today = new Date()): { actionable: SelectedCandidate[]; suppressed: { unchanged: number; resolved: number } } { const actionable = selectCandidates(scan, records, toolNames, today); const ids = new Set(actionable.map((x) => x.candidateId)); const map = latest(records); let unchanged = 0, resolved = 0; for (const c of scan.candidates) { if (toolNames && !toolNames.has(c.tool)) continue; if (ids.has(c.candidateId)) continue; const d = map.get(c.candidateId); if (!d) continue; if (d.disposition === "addressed") resolved++; else unchanged++; } return { actionable, suppressed: { unchanged, resolved } }; }
