	...
	registerSlashCommand(pi)("plan-it", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.cwd) {
				...
				workspaceDirective = `\n\nPRIMARY REPOSITORY (mandatory): ${activePlanningRoot}\nWrite the canonical plan directly under ${path.join(activePlanningRoot, ".specs", "<meaningful-slug>", "plan.md")}. Choose a concise kebab-case slug from the requested outcome and conversation context; never use an invocation ID or generic plan name. Do not create a planning worktree. Git state and repository-discovery failures never block planning; record relevant execution constraints in the plan for /do-it. The plan must require /do-it to perform implementation, test authoring, integration, validation, archive, and commit in its owned worktree. By default, finish implementation, test authoring, and integration first, then have the root run one final validation phase with each necessary command once. An explicit operator request for early or TDD validation may override that order and must be recorded in the plan. Merge and cleanup are the default; when the operator explicitly requests commit-and-retain closeout, record the exact Retention policy marker and require no merge. Every adversary, specialist, and proponent reviewer prompt must include the Verification-design rubric from plan-it.md and require supported findings to name the task key, failed rubric item, and proposed rewrite.`;
			}
