...
export function formatSubagentStatus(
	...
	return [
		...
		`process outcome: ${run.processOutcome ?? "pending"}`,
		`deliverable outcome: ${run.deliverableOutcome ?? "pending"}`,
	...
	return inspections
		.map((inspection) => {
			...
			return [
				...
				`processOutcome=${run.processOutcome ?? "pending"}`,
				`deliverable=${run.deliverableOutcome ?? "pending"}`,
